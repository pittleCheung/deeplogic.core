export default class CodeEditor {
  ...
  initElements() {
    ...
    this.textAreaElement.addEventListener('focus', (event) => {
      this.update();
    })
    this.textAreaElement.addEventListener('keydown', (event) => {
        ...
        switch (event.key) {
          case 'Tab':
            if (!this.autoCompletionOpened) {
              if (event.shiftKey) {
                if (
                  !this.hasSelection() &&
                  this.value.substring(this.getCaretPosition() - this.tab.length, this.getCaretPosition()) === this.tab
                ) {
                  const newCaretPosition = this.getCaretPosition() - 4;
                  this.value =
                    this.value.substring(0, this.getCaretPosition() - 4) + this.value.substring(this.getCaretPosition(), this.value.length);
                  this.setCaretPosition(newCaretPosition);
                }
              } else {
                const newCaretPos = caretPos + this.tab.length;
                this.value =
                  this.value.substring(0, this.getCaretPosition()) + this.tab + this.value.substring(this.getCaretPosition(), this.value.length);
          ...
          case 'Backspace':
            if (!this.hasSelection() && this.value.substring(this.getCaretPosition() - this.tab.length, this.getCaretPosition()) === this.tab) {
              ...
              this.value =
                this.value.substring(0, this.getCaretPosition() - 3) + this.value.substring(this.getCaretPosition(), this.value.length);
              this.setCaretPosition(newCaretPosition);
            }
      ...
      if (event.key === 'Enter') {
        if (caretPos === this.value.length) this.textAreaElement.scrollBy(0, this.textAreaElement.scrollHeight);
      }
      ...
      if (this.closeKeys !== false) {
        for (const key in this.closeKeys) {
          ...
          if (event.key === key) {
            ...
            if (this.hasSelection()) {
              ...
              this.value =
                this.value.substring(0, this.textAreaElement.selectionStart) +
                key +
                this.value.substring(this.textAreaElement.selectionStart, this.textAreaElement.selectionEnd) +
                this.closeKeys[key] +
                this.value.substring(this.textAreaElement.selectionEnd, this.value.length);
        ...
        if (event.key === 'Enter' && !this.hasSelection()) {
          ...
          if (['{', '(', '['].includes(char) || ('>' === char && this.value.substring(caretPos, caretPos + 1) === '<')) {
            this.value =
              this.value.substring(0, caretPos) +
              '\n' +
              this.tab +
              startingSpaces +
    ...
    this.textAreaElement.addEventListener('keyup', (ev) => {
      if (ev.key === 'Escape') this.closeAutocompletion();
  ...
  async checkAutoCompletion() {
    if (this.autoCompleteHandler === null) return;
    ...
    } /*if (completionsWord instanceof AutoCompletion)*/ else {
      completionsWord = await this.autoCompleteHandler.autoComplete(lastWord, this);
    }

    this.autoCompletionElement.innerHTML = '';

    if (completionsWord.length > 0) {
      ...
      for (const completion of completionsWord) {
        ...
        // reset autoCompletionIndex in arrow down
        if (this.autoCompletionIndex > completionsWord.length - 1) this.autoCompletionIndex = 0;
        // reset autoCompletionIndex in arrow up
        if (this.autoCompletionIndex < 0) this.autoCompletionIndex = completionsWord.length - 1;

        // Scroll to selected element
        if (this.autoCompletionIndex === i) {
          const {top: pt} = this.autoCompletionElement.getBoundingClientRect();
          const {top: ct} = el.getBoundingClientRect();
          if (pt && ct) {
            this.autoCompletionElement.scrollTop += ct - pt;
          }
  ...
  update() {
    if (!this.lineNumberingDisabled) this.updateLineNumbering();
    this.codePreElement.innerHTML = this.highlighter(this.value);

    const leftPadding = (this.lineNumberingDisabled ? 0 : this.lineNumerationsElement.offsetWidth || 33) + this.codeRightPadding;
