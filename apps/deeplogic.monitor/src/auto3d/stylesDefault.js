 ...
 * 文本框位置
 */
export const textTransform = ({ translateX, translateY, xAxis = 0, yAxis = 0 }) => {
  return {
...
export const textSystemTransform = (data, tag, other) => {
  return {
    translateX: data.props.style.translateX + data.props.style.width * 0.5 - (tag ? 8 + 10 : 3) + (other || 0),
    translateY: data.props.style.translateY + (tag ? 13 : 25 * -1 - 7)
  };
};
