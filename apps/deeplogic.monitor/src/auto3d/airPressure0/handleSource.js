...
import { bottomid, box, boxid, device, footid, headid, parentbox, pipe, root, rootHeader, text } from '../nodesDefault';
...
export const handleSource = (source, deviceModelMap, links, global, allsource) => {
  ...
  // console.log("maxHeight====>", maxHeight, ConHeight, gapY > 160, pGap.acopYGap)
  ...
  ACOPSARR.forEach((item, index) => {
    ...
    item.forEach((t, i) => {
      ...
      //   pipeh2.props.style.width = fix(pGap.acopXGap - (pipeh2.props.style.width / 3) * 2);
      ...
      if (i === len >> 1) {
        ...
        pipev1.props.style.height = styleMap.Acop.height + pGap.acopYGap + styleMap.h.height;
        ...
        pipev.props.style.height = styleMap.Acop.height + pGap.acopYGap + styleMap.h.height;
      ...
      if (i == 0) {
        ...
        if (curlen === nextlen) {
          ...
          // 空压设备数量大于下一个设备数量
          newInitTop = newInitTop + (styleMap.Acop.height + pGap.acopYGap) * (curlen - nextlen) * 0.5;
        }
        helpFunction({
