...
export const root = {
  ...
  props: {
    ...
    style: {
      ...
      margin: 0,
      background:
        "url(/assets/images/bg-light.webp) 50% 50% / 100% 100% no-repeat,linear-gradient(rgba(36, 66, 105, 1), rgba(36, 66, 105, 1))",
      borderRadius: 0,
      backgroundList: [
        {
          id: "EKzxxXgFQm",
  ...
  "8iFl32LjG1": {
    ...
    props: {
      ...
      style: {
        ...
        heightUnit: "px",
        background:
          "url(/assets/images/header-bg.png) 50% 50% / 100% 100% no-repeat",
        backgroundList: [
          {
            id: "pgueloUMWV",
            type: "image",
...
export const chl_pump = (item, result, type, dev, pipeType, pimps, index) => {
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    const pointarr = []
    const totalarr = []
    const totalstatus = checkTotal(
      deviceIdsMapValue,
      item,
      pimps,
      dev == "CTS" ? valuesMaps : undefined,
      totalarr,
    )
    ...
    if (dev == "CTS" && values.length > 1) {
      const value = device(
        ...
        valuesMaps?.[item[i]]?.[1],
        undefined,
        valuesModel[valuesMaps?.[item[i]]?.[1].ID],
      )
      value.props.style = {
        ...styleMap["Valve"],
        translateX: fix(pipeh1.props.style.translateX + 20),
        translateY: fix(
          pipeh1.props.style.translateY - styleMap["Valve"].height / 2 - 3,
        ),
      }
    ...
    if (i === item.length - 1) {
      ...
      if (i === 0) {
        ...
        connW.props.style = {
          ...
          translateX: co_f.translateX - styleMap["v"].width * 0.5 * tag + 1.5,
          translateY: fix(co_f.translateY - styleMap.h.height * 0.5) + 1.5,
          rotate: tag == 1 ? 0 : -90,
        }
...
export const chl_Out = (result, index, tag, current, item, CHLS) => {
  ...
  const translateX = fix(
    current.translateX + current.width - styleMap["t"].width,
  )
  // 纵管的x位移
  ...
  if (values.length > 0) {
    ...
    const resultMap = device(
      "Valve",
      valuesMaps?.[currentID]?.[0],
      undefined,
      valuesModel[valuesMaps?.[currentID]?.[0].ID],
    )
  ...
  if (index === 0) {
    ...
    pipeh2.props.style = {
      ...
      translateY: fix(pipeh3.props.style.translateY + 1.5),
  ...
  if (!!tag) {
    ...
    pipeh3.props.style = {
      ...styleMap["w"],
      ...connect,
      translateX: connect.translateX - .5, // 计算最后一个弯头向左偏移了1px 向下偏移了3px
      translateY: fix(
        connect.translateY + styleMap["w"].height + styleMap.h.height * 0.5 + 6.5,
      ),
    }
    result[pipeh3.id] = pipeh3
  }
...
export const pump_chl = (item, result, type, init, dType, pipeType, index) => {
  ...
  let pipewidth =
    type === "chw" && dType === "tower"
      ? styleMap["h"].width * 0.5
      : styleMap["h"].width * 0.5
  const initpos = fix(
    result[deviceMapIds[init]].props.style.translateY +
      (result[deviceMapIds[init]].props.style.height * 2) / 3,
  ) // 当前组总管的位置
  /**
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    if (Math.abs(vnum) > 0 && dType !== "tower") {
      ...
      let pipvArr = []
      if (i !== 0) {
        pipvArr = [...item.slice(0, i)]?.map((t) => deviceIdsMapValue[t])
      }
      ...
      // console.log('pipev================>', i, pipev.props.status.bind, ifTotal, deviceIdsMapValue, pipvArr, pointarr);
      if (
        i == 0 &&
        currentpump.translateY - initpos >= deviceMargin.Pump &&
        currentpump.translateY - initpos <
          deviceMargin.Pump + styleMap.CHWPS.height
      ) {
        const connW = connector("w")
        connW.props.style = {
    ...
    if (dType === "tower") {
      ...
      if (values.length > 0) {
        ...
        const value = device(
          ...
          valuesMaps?.[item[i]]?.[0],
          undefined,
          valuesModel[valuesMaps?.[item[i]]?.[0].ID],
        )
        value.props.style = {
          ...styleMap["Valve"],
          translateX: fix(pipeh1.props.style.translateX + 20),
    ...
    const connectXT = fix(
      translateX -
        styleMap["t"].width * tag -
        5 * tag +
        (type === "cw" ? styleMap.h.width + 2 : styleMap.t.height),
    )
    const connectXW = fix(
      translateX -
        (dType === "tower"
          ? styleMap.v.width * 0.5 * -1
          : styleMap["w"].width) -
        styleMap.v.width -
    ...
    const connectYT = fix(
      currentpump.translateY + styleMap.CHWPS.height - styleMap["t"].height,
    )
    // 下边两侧的三头 和 连接总管的一条竖管线和一个弯头
    if (i == item.length - 1) {
      const wtag =
        dType !== "tower" &&
        Math.abs(currentY - initpos) <= deviceMargin.Pump - 2
    ...
    if (dType !== "tower") {
      if (i === 0) {
        ...
        // 其他泵
        if (
          Math.floor(Math.abs(vnum)) == 0 ||
          (Math.abs(currentY - initpos) <= deviceMargin.Pump &&
            i < item.length - 1)
        ) {
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    if (i == 0) {
      ...
      connW.props.style = {
        ...
        translateY:
          pipeh1.props.style.translateY - styleMap["w"].height / 2 + 4,
      }
      result[connW.id] = connW
      ...
      connW.props.style = {
        ...
        translateX:
          pipeh1.props.style.translateX +
          pipeh1.props.style.width +
          styleMap["w"].width * 0.5 - styleMap["w"].width * 0.2,
    ...
    if (i > 0) {
      ...
      connT0.props.style = {
        ...
        // 调整三头的位置
        translateX: conTranslateX - 1.5,
        translateY:
          pipeh1.props.style.translateY - styleMap["h"].height / 2 - 1,
        rotate: 90,
      }
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    if (values.length > 1) {
      ...
      value.props.style = {
        ...styleMap["Valve"],
        translateX: fix(
          pipeh.props.style.translateX +
            styleMap["h"].width * 0.5 -
            20 -
            styleMap["Valve"].width,
        ),
        translateY: fix(
          pipeh.props.style.translateY - styleMap["Valve"].height / 2 - 3,
        ),
      }
      result[value.id] = value
    }
    ...
    if (i == 0) {
      ...
      pipeh0.props.style = {
        ...
        translateY: fix(
          currentpump.translateY - 50 + styleMap["CHWPS"].height / 3,
        ),
      }
      ...
      connW.props.style = {
        ...styleMap["w"],
        translateX:
          pipeh0.props.style.translateX +
          pipeh0.props.style.width -
          styleMap.w.width / 2 - 1.5,
    ...
    if (i !== item.length - 1) {
      ...
      connW.props.style = {
        ...styleMap["w"],
        translateX: fix(transObj.translateX - styleMap["w"].width / 2) + 2,
