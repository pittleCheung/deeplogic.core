  ...
  ACOPSARR.forEach((item, index) => {
    ...
    item.forEach((t, i) => {
      ...
      if (i === len) {
        ...
        result[pipeh2.id] = pipeh2
        if (item.length > current.NEXT_NODE.length) {
          const pipev = pipe("v", "1")
          pipev.props.waterstyle = "1"
          pipev.props.style = { ...styleMap["v"], fill: "#407FCB" }
          // 相对于总管横管的位置计算总管竖管X轴位移 = 总管横管X位移+总管横管宽度
          pipev.props.style.translateX =
            pipeh2.props.style.translateX + pipeh2.props.style.width
          // 总管竖管高度 = 总管横管高度 + 数量差*间隔
          const diff = (item.length - current.NEXT_NODE.length) * pGap.acopYGap
          pipev.props.style.height =
            pipeh2.props.style.height + diff + styleMap["v"].width * 2
          // 总管竖管Y轴位移 = 横管Y轴位移 - 间隔
          pipev.props.style.translateY =
            pipeh2.props.style.translateY - diff - styleMap["v"].width * 2
          pipev.props.status = {
            bind: str.slice(0, -2),
            type: "expressions",
          }
