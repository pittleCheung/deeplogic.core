    ...
    if (Array.isArray(t)) {
      t.forEach((item, index) => {
        ...
        if (
          tag === "R" &&
          index === t.length - 1 
          && current.NEXT_NODE.length > 0 // 储气干罐不走这里
        ) {
          // 生成总管横管
          pipeh2.props.waterstyle = "1"
          pipeh2.props.style = { ...styleMap["h"], fill: "#407FCB" }
          ...
          pipeh2.props.style.translateX = target.translateX + target.width + pipeh1.props.style.width - 20
          pipeh2.props.style.translateY = pipeh1.props.style.translateY
          if (current?.ONOFF) {
            pipeh2.props.status = {
              bind: `${"${" + current.ONOFF.NAME + "}"}==1`,
          ...
          // 前面的数量大于后面的数量需要添加一根竖管
          if (t.length > current.NEXT_NODE.length) {
               const pipev = pipe("v", "1")
               pipev.props.waterstyle = "1"
               pipev.props.style = { ...styleMap["v"], fill: "#407FCB" }
               // 相对于总管横管的位置计算总管竖管X轴位移 = 总管横管X位移+总管横管宽度
               pipev.props.style.translateX = pipeh2.props.style.translateX + pipeh2.props.style.width;
               // 总管竖管高度 = 总管横管高度 + 数量差*间隔
               const diff = (t.length - current.NEXT_NODE.length) * pGap.acopYGap 
               pipev.props.style.height = pipeh2.props.style.height + diff
               // 总管竖管Y轴位移 = 横管Y轴位移 - 间隔
               pipev.props.style.translateY = pipeh1.props.style.translateY - diff
        ...
        if (tag === "L" && lastLen === 1 && index === t.length - 1 
          ...
          if (current.NO === current.LEN && t.length < current.NEXT_NODE.length) {
            // 横管总管
            ...
            pipev.props.style = { ...styleMap["v"], fill: "#407FCB" }
            // 相对于总管横管的位置计算总管竖管X轴位移 = 总管横管X位移+总管横管宽度
            pipev.props.style.translateX =
              pipeh3.props.style.translateX + pipeh3.props.style.width
            // 总管竖管高度 = 总管横管高度 + 数量差*间隔
            const diff =
              (current.LEN - current.NEXT_NODE.length) * pGap.acopYGap
            pipev.props.style.height = pipeh3.props.style.height + diff
            // 总管竖管Y轴位移 = 横管Y轴位移 - 间隔
            pipev.props.style.translateY = pipeh3.props.style.translateY - diff
            result[pipev.id] = pipev
            if (current?.preStates) {
              pipev.props.status = {
                bind: current?.preStates.slice(0, -2),
        ...
        if (current.TYPE) {
          ...
          if (
            current.TYPE === "D0C1E" &&
            (current.NO === 1 || current.NO === current.LEN)
          ) {
