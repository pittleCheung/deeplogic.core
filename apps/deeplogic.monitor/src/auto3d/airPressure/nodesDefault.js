...
export const position = ({ index, initTop, prevX, laststyle }) => {
  ...
  return {
    ...
    translateY: (index + 1) * (styleMap.Acop.height + pGap.acopYGap) + initTop,
    ...
    // 80为默认的初始化的位移
    translateX: laststyle ? prevX + laststyle.props.style.width + pGap.acopXGap : 80
  ...
  Object.values(obj).forEach((t, i) => {
    if (Array.isArray(t)) {
      t.forEach((item, index) => {
        ...
        // 2. 直接利用空压机的高度算
        pipeh1.props.style.translateY = target.translateY + styleMap.Acop.height * 0.65;
        ...
        let vheight = len % 2 == 0 ? styleMap.Acop.height + pGap.acopYGap : (styleMap.Acop.height + pGap.acopYGap) * 0.5;
        ...
        if (
          (tag === 'R' && index === (t.length - 1) >> 1 && (objle > 1 ? i === (objle - 1) >> 1 : 1)) ||
        ...
        if (i == 0 && arr?.[0]?.NEXT_NODE?.length > 1 && arr?.[0]?.NEXT_NODE?.some((t) => !childMap.get(t.ID))) {
          ...
          helpFunction({
            ...
            initTop: initTop + Math.abs(curlen - nextlen) * (styleMap.Acop.height + pGap.acopYGap) * 0.5,
        ...
        if (current.TYPE) {
          ...
          // target.translateY + target.height * 0.65 + 10 * 0.65
          // 2. 直接利用空压机的高度算
          pipehr1.props.style.translateY = target.translateY + styleMap.Acop.height * 0.65;
          ...
          if (
            (current.TYPE === 'D0C1E' || (arr?.[i]?.NEXT_NODE?.length > 1 && (index != t.length - 1 || t.length === 1))) &&
